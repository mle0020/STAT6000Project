...
Also, we create the boxplot of each predictor and check the difference between quality 0 and 1 in each variable. Except for `residual.sugar`, `chlorides`, `free.sulfur.dioxide`, and `PH`, there are obvious different between "good" and "bad" wines. In addition, the box-plots show that there are a lot of outliers in different predictors, and it could be the reason of misclassification.
...
```{r KNN}
train -> winequality.red[trainset,]
test-> winequality.red[-trainset,]
train_target-> winequality.red[1:1599,12]
knn.pred->knn(train,test,train_target,k=10)
```
